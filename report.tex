...
To test whether the failure is attributable to the distance metric rather than the clustering criterion, we replace Euclidean with Gower distance (range-normalized Manhattan for continuous, matching dissimilarity for binary) on the same 2,500-row sample.  Ward + Gower yields a best ARI of 0.215 at $k=7$ — still far below supervised F1 — confirming that the bottleneck is the clustering criterion, not the distance function.
