...
\end{equation}
where $\mu$ and $\sigma$ are estimated exclusively from the sub-sample used for each specific analysis (t-SNE: 6,000; clustering: 2,500--6,000; supervised: 68,565 training), not from the full 581,012-observation dataset.  We note a minor procedural caveat in the supervised pipeline: the \texttt{StandardScaler} is fitted on all 97,950 modeling observations before the train--test split, so $\mu$ and $\sigma$ incorporate information from the test observations as well.  Because the scaler uses only feature means and variances — not label information — and training and test distributions are drawn from the same stratified sample, the resulting information leakage is negligible.  In a strict production setting, one should fit the scaler exclusively on \texttt{X\_train} and apply it via \texttt{transform} to \texttt{X\_test}. Binary wilderness and soil indicators remain in $\{0,1\}$. Tree models receive the same predictors without requiring scale normalization. Stratification is used whenever a sample or train-test split is drawn, preserving rare classes such as Cottonwood/Willow. Finally, continuous columns are stored as \texttt{float32} and indicators as \texttt{uint8}; the resulting dataframe requires about 47.1 MB rather than the 244 MB estimated for an all-\texttt{int64} representation. This reduction makes repeated analysis practical without altering the underlying values.
...
\begin{table}[H]
  ...
  \begin{tabular}{lrrrr}
    ...
    Agglomerative Ward & 4 & 0.156 & 389.500 & 0.061 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\label{eq:impurity}
\end{equation}
where $\mathcal{N}_t^{(j)}$ contains nodes in tree $t$ that split on feature $j$, and $\Delta I_n$ is the resulting impurity reduction. Figure~\ref{fig:importance}a ranks Elevation first (0.237), followed by distances to roads, fire points, and hydrology. Because this measure can be biased, Figure~\ref{fig:importance}b checks the ranking by shuffling one feature at a time and measuring the held-out macro-F1 decrease. It independently confirms Elevation as dominant, producing a 0.358 mean F1 drop, followed by road distance (0.140) and fire-point distance (0.103). Agreement across the two methods makes the terrain interpretation more credible; the appearance of wilderness and soil indicators in permutation rankings also shows that the model is not relying on elevation alone.
...
Figure~\ref{fig:calibration_errors}b connects confidence to actual errors. Correct Random Forest predictions have mean confidence 0.810 and median confidence 0.841; incorrect predictions fall to a mean of 0.582 and median of 0.565. Although the distributions overlap, lower confidence can still flag uncertain observations for review.  A paired McNemar test between Random Forest and HistGradientBoosting on the 29,385-row test set yields $\chi^2(1)=41.5$, $p<0.001$, confirming that the performance difference between the two best classifiers is statistically significant despite the close absolute scores.
