...
Figure~\ref{fig:calibration_errors}b connects confidence to actual errors. Correct Random Forest predictions have mean confidence 0.811 and median confidence 0.844; incorrect predictions fall to a mean of 0.586 and median of 0.569. Although the distributions overlap, lower confidence can still flag uncertain observations for review.  A paired McNemar test between Random Forest and HistGradientBoosting on the 29,385-row test set yields $\chi^2(1)=41.5$, $p<0.001$, confirming that the macro-F1 difference is statistically detectable despite the close absolute scores (0.908 vs.\ 0.898).  Converting this difference to Cohen's $h$ gives $h \approx 0.04$, far below the conventional threshold for a small effect ($h=0.20$), which reinforces that the detectable difference is practically negligible and driven by the large test-set size rather than a meaningful performance gap.
